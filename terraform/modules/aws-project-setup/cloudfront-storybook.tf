resource "aws_cloudfront_origin_access_control" "storybook" {
  name                              = aws_s3_bucket.storybook.id
...
resource "aws_cloudfront_distribution" "storybook" {
  ...
  origin {
    ...
    origin_id                = aws_s3_bucket.storybook.bucket_regional_domain_name
  ...
  default_cache_behavior {
    target_origin_id       = aws_s3_bucket.storybook.bucket_regional_domain_name
...
output "storybook_cdn" {
  value = aws_cloudfront_distribution.storybook
}
