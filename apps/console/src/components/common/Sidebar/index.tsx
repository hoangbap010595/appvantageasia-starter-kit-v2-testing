import { Button, CloseButton, type ButtonProps } from '@headlessui/react';
import { clsx } from 'clsx';
import { LayoutGroup, motion } from 'framer-motion';
import { forwardRef, useId, type ComponentPropsWithoutRef, type ReactNode, type ForwardedRef } from 'react';
import TouchTarget from '@/components/common/Button/TouchTarget';
import Link from '@/components/common/Link';

export const Sidebar = ({ className, ...props }: ComponentPropsWithoutRef<'nav'>) => {
    return <nav {...props} className={clsx(className, 'flex h-full min-h-0 flex-col')} />;
};

export const SidebarHeader = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => {
    return (
        <div
            {...props}
            className={clsx(
...
export const SidebarBody = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => {
    return (
        <div
            {...props}
            className={clsx(
...
export const SidebarFooter = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => {
    return (
        <div
            {...props}
            className={clsx(
...
export const SidebarSection = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => {
    const id = useId();
...
export const SidebarLabel = ({ className, ...props }: ComponentPropsWithoutRef<'span'>) => {
